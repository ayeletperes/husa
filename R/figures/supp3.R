...
source("R/00_setup.R")
suppressPackageStartupMessages({ library(ggplot2); library(ggseqlogo); library(patchwork); library(ComplexUpset); library(Biostrings); library(alakazam) })
set.seed(42)
...
fwrite(comp[, .(gene_type, leader, HUSA, IMGT, present, count_genomic)], file.path(OUT$source, "supp3_overlap.csv"))
fwrite(overlap_counts, file.path(OUT$source, "supp3_overlap_counts.csv"))
fwrite(seqlogo_input, file.path(OUT$source, "supp3_seqlogo_input.csv"))
fwrite(consensus_counts, file.path(OUT$source, "supp3_consensus_counts.csv"))
