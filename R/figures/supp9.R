...
tables <- file.path(OUT$source, "supp9_allele_counts.csv.gz")
if (!all(file.exists(tables))) {
  ...
  fwrite(plot_data, tables[1])
}

plot_data <- fread(tables[1])
plot_data[, tag_display := factor(tag_display, levels = tag_levels)]
